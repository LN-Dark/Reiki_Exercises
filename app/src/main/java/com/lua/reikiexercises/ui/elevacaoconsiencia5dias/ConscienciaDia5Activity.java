...
public class ConscienciaDia5Activity extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_consciencia_dia5);
        ...
        TextInputEditText txt_observacao = findViewById(R.id.edittext_consciencia_dia5_observacao_input);
        MaterialButton btn_save = findViewById(R.id.btn_fechar_consciencia_dia5);
        SharedPreferences prefs = getSharedPreferences("ReikiExercises", MODE_PRIVATE);
        String pergunta1 = prefs.getString("ReikiExercises_consciencia_Dia5_observacao", "");
        if(!pergunta1.equals("")){
        ...
        btn_save.setOnClickListener(v -> {
            ...
            if(!txt_observacao.getText().toString().equals("")){
                editor.putString("ReikiExercises_consciencia_Dia5_observacao", txt_observacao.getText().toString());
            }else{
                editor.putString("ReikiExercises_consciencia_Dia5_observacao", "");
            }
            editor.apply();
            finish();
        });
